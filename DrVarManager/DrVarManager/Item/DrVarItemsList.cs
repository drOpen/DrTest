/*
  DrVarItemsList.cs -- list of variables for 'DrOpen.DrVariable' general purpose Builder variables 1.0, June 22, 2014
...
namespace DrOpen.DrVariable.Item
{
    /// <summary>
    /// list of variables
    /// </summary>
    internal class DrVarItemsList : IEnumerable<DrVarItem>
    {
        ...
        {
            get { return DrVarConst.varSymbol.ToString(); }
        }

        public string EscapeVarSymbol
        {
            get { return DrVarConst.escapeVarSymbol; }
        }
        ...
        internal void Parse(string value)
        {
            ...
            if (value.Contains(DrVarConst.varSymbol.ToString())) //Exit if the string does not contain a variable indicating symbol.
            {
                ...
                foreach (char ch in value) // enumerating all characters in the string
                {
                    ...
                    if (ch == DrVarConst.varSymbol)
                    {
                        if (isVarNameStarted)
                        {
                            varItems.Add(new DrVarItem(iCurrentPosition - varName.Length - 2, iCurrentPosition, varName, DrVarConst.varSymbol + varName + DrVarConst.varSymbol)); //add new variable item to list
                ...
                if (OpenedVarCounter != (ClosedVarCounter + EscapeVarSymbolCounter * 2)) throw new FormatException(string.Format(Res.Msg.CANNOT_BUILD_VAR_NOT_CLOSED_SYMBOL, value, DrVarConst.varSymbol));
            }
        }
